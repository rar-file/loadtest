...
if TYPE_CHECKING:
    from loadtest.core import LoadTest, TestResult
...
class ProgressTracker:
    ...
    def _create_stats_table(self, stats: LiveStats) -> Table:
        ...
        table.add_row("Error Rate", f"[red]{stats.error_rate:.1f}%[/red]" if stats.error_rate > 5 else f"{stats.error_rate:.1f}%")
    ...
    async def _update_loop(self, live: Live, progress: Progress, task_id: int) -> None:
        ...
        while not self._stop_event.is_set():
            elapsed = time.time() - (self.start_time or time.time())
            percentage = min(100, (elapsed / self.duration) * 100)
...
def show_test_summary(result: TestResult, console: Console | None = None) -> None:
    ...
    table.add_row("Failed", f"[red]{result.failed_requests:,}[/red]" if result.failed_requests > 0 else "0")
    table.add_row("Success Rate", f"[green]{result.success_rate:.1f}%[/green]" if result.success_rate >= 95 else f"[yellow]{result.success_rate:.1f}%[/yellow]")
