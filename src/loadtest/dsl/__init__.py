...
def http_post(
    ...
    config = HTTPConfig(
        method="POST",
        url=url,
        headers={**(headers or {}), 'Content-Type': 'application/json'} if json_data else (headers or {}),
        json_data=json_data,
...
def spike(baseline_rps: float, spike_rps: float, spike_duration: float, interval: float) -> dict[str, Any]:
    """Spike load pattern.
