...
def find_test_function(module: Any) -> Any:
    ...
    raise AttributeError("Configuration module must define 'create_test', 'main', or 'test'")
...
async def run_test(
    config_path: str, output: str | None = None, duration: float | None = None
) -> int:
...
def show_info() -> None:
    ...
    table.add_row(
        "Scenario", "AuthenticatedHTTPScenario", "HTTP with Bearer/API key authentication"
    )
    table.add_row(
        "Scenario", "WebScenario", "Browser automation with Playwright (requires [web] extra)"
    )
