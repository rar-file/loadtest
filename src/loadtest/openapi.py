...
class OpenAPIDetector:
    ...
    def _generate_sample_from_schema(self, schema: dict[str, Any]) -> dict[str, Any]:
        ...
        if schema_type == "object":
            ...
            for prop_name, prop_schema in properties.items():
                if prop_name in required or len(properties) <= 5:  # Include non-required if few props
    ...
    def generate_loadtest_config(self, max_endpoints: int = 10) -> dict[str, Any]:
        ...
        endpoints.sort(key=lambda e: priority_methods.index(e["method"]) if e["method"] in priority_methods else 99)
