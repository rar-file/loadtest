...
class TrafficPattern(ABC):
    ...
    def on(
        ...
        Callable[[PatternEvent], None]
        | Callable[[Callable[[PatternEvent], None]], Callable[[PatternEvent], None]]
    ):
...
class StepLadderGenerator(TrafficPattern):
    ...
    def __init__(
        ...
        if direction == "up":
            self._step_values = [
                start_rate + (end_rate - start_rate) * i / (steps - 1) for i in range(steps)
            ]
        elif direction == "down":
            self._step_values = [
                end_rate + (start_rate - end_rate) * i / (steps - 1) for i in range(steps)
            ]
        else:  # updown
